...
class TestV(unittest.TestCase):
    ...
    def test_token(self):
        ...
        assert self.token_api.is_ok(self.token_api.search_one(query="QuanticPotato"))
        ...
        agent = self.key_api.whoami()
        #result = self.token_api.bulk_info([agent.telegramid], array=True, telegramid=True)
        #assert type(result) is list
        ...
        result = self.token_api.team_details(self.token_api.list_teams()[0].teamid)
        assert type(result) is list
